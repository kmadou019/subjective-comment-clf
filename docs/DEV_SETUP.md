...
This guide covers how to set up your environment and run this project on the
lab's GPU cluster (SSH access, the OAR job scheduler, a root-less Ollama
install, and a Hugging Face token for embeddings). For a high-level overview
of the project and its architecture, see the [main README](../README.md).
...
![Menu to access Hugging Face tokens](../documentation/img/access-token.png)

Click the **Create new token** button:

![Create new token button](../documentation/img/create-token.png)

Then, generate a new token and make sure the token type is set to **write**:

![Selecting 'write' as token type](../documentation/img/write.png)
...
![Training set format example](../documentation/img/trainset.png)
...
![Prompt section in graph_builder.py](../documentation/img/prompt.png)

### Modify the Number of Retrieved Documents

To change `k`, the number of documents retrieved during inference, open the file `graph_builder.py` and locate the line that sets the value of `k` in the `retrieve` function, as shown in the figure below. Then put the value you want.

![Modification of k in graph_builder.py](../documentation/img/k.png)
...
![Contents of rag.sh](../documentation/img/run.png)
